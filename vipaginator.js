...
angular.module("viPaginator", []).directive('viPaginator', function () {
    return {
        scope: {
            itemsArray: '=',
            newArray: '=',
        ...
        controller: function ($scope, $http) {
            ...
            if (typeof $scope.itemsArray != 'undefined') {
                $scope.$watch("perPage", function (perPage) {
                    ...
                    $scope.newArray = $scope.itemsArray.slice(0, perPage);

                    var options = {
                        currentPage: 1,
                        totalPages: Math.ceil($scope.itemsArray.length / $scope.perPage),
                        itemTexts: function (type, page, current) {
                            switch (type) {
                                case "first":
                                    return "First";
                        ...
                        onPageClicked: function (e, originalEvent, type, page) {
                            $scope.$apply(function () {
                                ...
                                $scope.newArray = $scope.itemsArray.slice(startpoint, (parseInt(startpoint) + parseInt(perPage)));
                            });
                        }
                    };
